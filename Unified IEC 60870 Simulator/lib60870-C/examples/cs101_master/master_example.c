...
int main(int argc, char **argv) {
    ...
    const char *interface = readConfigValue("KONFIG101CLIENT.txt", "Interface");
    char *bandwidthStr = readConfigValue("KONFIG101CLIENT.txt","Bandwidth");
    char *commonAddressStr = readConfigValue("KONFIG101CLIENT.txt", "Common Address");
    char *periodStr = readConfigValue("KONFIG101CLIENT.txt", "PERIOD");
    int periodicInterval = periodStr ? atoi(periodStr)
                                     : 20;  // Defaultní perioda je 20 sekund, pokud není specifikováno jinak
    char *syncStr = readConfigValue("KONFIG101CLIENT.txt", "SYNC");
    int syncSwitch = syncStr ? atoi(syncStr) : 0;
    char *dataConfigStr = readConfigValue("KONFIG101CLIENT.txt", "DATALOGS");
    dataConfig = atoi(dataConfigStr);
    char *serviceConfigStr = readConfigValue("KONFIG101CLIENT.txt", "SERVICELOGS");
    serviceConfig = atoi(serviceConfigStr);
    servicePath = readConfigValue("KONFIG101CLIENT.txt", "SERVICEPATH");
    dataPath = readConfigValue("KONFIG101CLIENT.txt", "DATAPATH");
    ...
    while (running) {
        CS101_Master_run(master); // Ensure frequent execution to process incoming messages

        time_t currentTime = time(NULL);
        Thread_sleep(100); // Adjust sleep duration if necessary

        if (difftime(currentTime, lastSentTime) >= periodicInterval) {

            readMessageConfig("KONFIG101CLIENT.txt");
            for (int i = 0; i < numMessageConfigs; ++i) {
                ...
                for (int j = 0; j < msg.ioContentCount; ++j) {
                    ...
                    if (msg.messageType == 45 || msg.messageType == 46) {
                        CS101_Master_sendProcessCommand(master, CS101_COT_ACTIVATION, commonAddress, io);
            ...
            const char* config_file = "KONFIG101CLIENT.txt";
            ...
            if (syncSwitch == 1) {
                ...
                CS101_Master_sendClockSyncCommand(master, commonAddress, &newTime);
            }
            CS101_Master_sendInterrogationCommand(master, CS101_COT_ACTIVATION, commonAddress, IEC60870_QOI_STATION);
