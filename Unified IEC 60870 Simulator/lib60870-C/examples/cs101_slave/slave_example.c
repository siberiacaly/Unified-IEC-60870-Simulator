...
void sendSpontaneousMessage(CS101_Slave slave, CS101_AppLayerParameters alParameters, int multiplier) {
    CS101_ASDU newAsdu = CS101_ASDU_create(alParameters, true, CS101_COT_SPONTANEOUS, originatorAddress, commonAddress,
                                           false, false);
...
static bool interrogationHandler(void *parameter, IMasterConnection connection, CS101_ASDU requestAsdu, uint8_t qoi) {
    ...
    if (qoi == 20) {
        CS101_AppLayerParameters alParameters = IMasterConnection_getApplicationLayerParameters(connection);
        if (serviceConfig == 1) {
            LogRXrequest(qoi);
        }
        ...
        for (int i = 0; i < numMessageConfigs; i++) {
            ...
            if (msg.messageType < MAX_MESSAGE_TYPES) {
                if (!asdus[msg.messageType]) {
                    asdus[msg.messageType] = CS101_ASDU_create(alParameters, false, CS101_COT_INTERROGATED_BY_STATION,
...
int main(int argc, char **argv) {
    ...
    const char *interface = readConfigValue("KONFIG101SERVER.txt", "Interface");
    char *bandwidthStr = readConfigValue("KONFIG101SERVER.txt", "Bandwidth");
    int bandwidth = atoi(bandwidthStr);
    char *originatorAddressStr = readConfigValue("KONFIG101SERVER.txt", "Originator Address");
    char *commonAddressStr = readConfigValue("KONFIG101SERVER.txt", "Common Address");


    originatorAddress = originatorAddressStr ? atoi(originatorAddressStr) : -1;
    commonAddress = commonAddressStr ? atoi(commonAddressStr) : -1;

    if (originatorAddress < 0 || originatorAddress > 65535 || commonAddress < 0 || commonAddress > 65535) {
        fprintf(stderr, "Error: Invalid address values. Originator Address: %d, Common Address: %d\n", originatorAddress, commonAddress);
        exit(EXIT_FAILURE);
    }
    char *spontaneousConfig = readConfigValue("KONFIG101SERVER.txt", "SPONTANEOUS");
    char *multiplierStr = readConfigValue("KONFIG101SERVER.txt", "MULTI");
    readMessageConfig("KONFIG101SERVER.txt");
    char *dataConfigStr = readConfigValue("KONFIG101SERVER.txt", "DATALOGS");
    dataConfig = atoi(dataConfigStr);
    char *serviceConfigStr = readConfigValue("KONFIG101SERVER.txt", "SERVICELOGS");
    serviceConfig = atoi(serviceConfigStr);
    servicePath = readConfigValue("KONFIG101SERVER.txt", "SERVICEPATH");
    dataPath = readConfigValue("KONFIG101SERVER.txt", "DATAPATH");
    ...
    char *periodStr = readConfigValue("KONFIG101SERVER.txt", "PERIOD");
    ...
    printf("Connecting to interface: %s with bandwidth %d\n", interface, bandwidth);
    printf("Interface: %s\n", interface);
    printf("Bandwidth: %d\n", bandwidth);
    printf("Originator Address: %d\n", originatorAddress);
    printf("Common Address: %d\n", commonAddress);

    SerialPort port = SerialPort_create(interface, bandwidth, 8, 'E', 1);
    ...
    if (SerialPort_open(port)) {
        ...
        exit(EXIT_FAILURE); // Exit the program if the serial port cannot be opened
    }
    ...
    while (running) {
        ...
        if (difftime(currentTime, lastSentTime) >= periodicInterval) {
            ...
            for (int i = 0; i < numMessageConfigs; ++i) {
                MessageConfig msg = messageConfigs[i];
                CS101_ASDU asdu = CS101_ASDU_create(alParameters, false, CS101_COT_PERIODIC, originatorAddress, commonAddress, false, false);

                for (int j = 0; j < msg.ioContentCount; ++j) {
                    IOContent ioContent = msg.ioContent[j];
                    InformationObject io = createIO(msg.messageType, ioContent.ioa, ioContent.value);
                    if (!io) {
                        fprintf(stderr, "Failed to create InformationObject\n");
                        continue;
                    }
        ...
        if (spontaneousEnabled && currentTime >= nextSpontaneousTime) {
            for (int i = 0; i < multiplier; i++) {
                sendSpontaneousMessage(slave, alParameters, multiplier);  // Now correctly passing all required parameters
            }
